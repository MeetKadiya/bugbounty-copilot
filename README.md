...
- **Modular recon pipeline** (10 stages, each independently swappable):
  ...
  10. Secret detection (AWS, GCP, Firebase, JWT, Slack, Stripe, GitHub tokens, private keys, basic-auth URLs — all redacted)
...
- **Dark cyber-themed dashboard**: overview cards, live scan progress, asset graph, subdomain table, endpoint/parameter explorers, secrets panel, tech stack, AI recommendations, risk gauge, JSON/Markdown export.
...
│   ├── app/
│   │   ├── api/              REST routes (targets, scans, findings, export)
│   │   ├── core/              scope validation, risk scoring
│   │   ├── scanners/          10 modular scanner classes (BaseScanner subclasses)
...
│   ├── tests/                 pytest unit tests (scope, risk score, secrets, API)
